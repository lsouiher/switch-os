...
export default function Terminal({ windowId }: { windowId: string }) {
  const [lines, setLines] = useState<TerminalLine[]>([
    { type: 'output', text: 'Welcome to SwitchOS Terminal' },
  ...
  const resolvePath = (path: string): string => {
    ...
    if (path.startsWith('~')) return '/Users/you' + path.slice(1);
  ...
  const executeCommand = (cmd: string) => {
    ...
    const newLines: TerminalLine[] = [
      ...lines,
      { type: 'input', text: `you@switchos ${cwd.replace('/Users/you', '~')} % ${trimmed}` },
    ];
    ...
    switch (command) {
      case 'ls': {
        ...
        if (!node) {
          newLines.push({ type: 'error', text: `ls: ${args[0] || path}: No such file or directory` });
      ...
      case 'cd': {
        if (!args[0] || args[0] === '~') {
          ...
          const node = getNodeByPath(path);
          if (!node) {
            newLines.push({ type: 'error', text: `cd: no such file or directory: ${args[0]}` });
          } else if (node.type !== 'folder') {
            newLines.push({ type: 'error', text: `cd: not a directory: ${args[0]}` });
          } else {
            setCwd(path);
          }
        }
      ...
      case 'mkdir': {
        if (!args[0]) {
          ...
          if (!parent) {
            newLines.push({ type: 'error', text: `mkdir: ${parentPath}: No such file or directory` });
          } else {
      ...
      case 'rm': {
        if (!args[0]) {
          ...
          const node = getNodeByPath(path);
          if (!node) {
            newLines.push({ type: 'error', text: `rm: ${args[0]}: No such file or directory` });
          } else {
            deleteNode(node.id);
          }
        }
      ...
      case 'mv': {
        if (args.length < 2) {
          ...
          if (!src) {
            newLines.push({ type: 'error', text: `mv: ${args[0]}: No such file or directory` });
          } else if (!dest || dest.type !== 'folder') {
            newLines.push({ type: 'error', text: `mv: ${args[1]}: Not a directory` });
          } else {
      ...
      case 'cp': {
        if (args.length < 2) {
          ...
          const dest = getNodeByPath(destPath);
          if (!src) {
            newLines.push({ type: 'error', text: `cp: ${args[0]}: No such file or directory` });
          } else if (!dest || dest.type !== 'folder') {
            newLines.push({ type: 'error', text: `cp: ${args[1]}: Not a directory` });
          } else {
      ...
      case 'help':
        newLines.push({
          type: 'output',
          text: 'Available commands: ls, cd, pwd, mkdir, touch, rm, mv, cp, cat, echo, clear, help',
        });
        break;
      default:
        newLines.push({ type: 'error', text: `zsh: command not found: ${command}` });
    }
      ...
      <div className="flex-1 overflow-auto p-3">
        {lines.map((line, i) => (
          <div
            ...
            style={{
              color: line.type === 'error' ? '#f87171' : line.type === 'input' ? '#22c55e' : '#d4d4d4',
            }}
          >
            {line.text}
          </div>
        ...
        <div className="flex items-center">
          <span style={{ color: '#22c55e' }}>
            you@switchos {cwd.replace('/Users/you', '~')} %{' '}
          </span>
          <input
            ref={inputRef}
            value={input}
